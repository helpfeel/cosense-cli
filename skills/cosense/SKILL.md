...
### ページ内の画像・添付ファイルの中身を確認したい時

画像・ファイルの読み方: [read-page.md](read-page.md) の「本文中の画像・ファイルを読む」を参照

- 本文中の `https://<host>/files/<fileId>.<ext>` 形式のURLは、projectにアップロードされたファイル（画像・PDF等）
- 本文中の `https://gyazo.com/<hash>` 形式のURLは、Gyazoにアップロードされた画像・動画（公開されている物のみ読める）
- その中身が回答に必要な時に参照する
